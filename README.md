...
### Test files
Two files `test.bam` & `testTE.gff` are provided within LocaTE for testing its operation. Once LocaTE and its dependences are in your `PATH` variable, you can execute it as follows. Normally, this process takes in seconds. Then, a output file `test.bam_TE` would be generated. 
```sh 
LocaTE -b test.bam -r testTE.gff
```
...
* <b>BAM file must be sorted.</b>
* Notably, our testing has been conducted solely with the <b>GFF file from RepeatMaske.</b> Thus, the TE annotation file shall be generated by RepeatMasker, or as follows：
```
...
```sh
	-b <BAM file (sorted)>
	-r <TE annotation file (GFF)>
	-o <Output prefix> (default: working directory)
	-f <Minimum overlap requirement> (default: 0.5)
	-p <Expected probability threshold for assignment> (default: 0.8)
...
```
## Advanced features
### Cleaning the TE annotation file (gff)
...
### Detecting abnormal expression of ERVs
LocaTE also comes with a python script that detects the abnormal expression of ERVs by `-e`. It statistics the ratio of each ERV between RNA-seq and genome. Based on DBSCAN, the ERVs with abnormal value are deemed as exogenous.
