...
# Or through a list of BAM files (generating TPM and ReadCount table)
LATTE -l <list.of.BAM.files> -g <TE.gff.by.RepeatMasker> 
```
[Important considerations](./README.md.md#general-usage):
* `-b` BAM file must be sorted by coordinate.
* `-l` List of <b>absolute paths</b> to sorted BAM files, one per line without any space.
* `-g` GFF file must be generated by [RepeatMasker](https://www.repeatmasker.org/).
...
* `-b` <b>BAM file must be sorted by coordinate </b> as follows:
    ...
    ```
* `-l` <b>List of BAM files shall be absolute paths, one per line without any space.</b> Example:
    ```
    /abs/path/to/bam_file_1
    /abs/path/to/bam_file_2
    ```
* `-g` Notably, our testing has been conducted solely with the <b>GFF file from [RepeatMasker</b>](https://www.repeatmasker.org/). 
    - Recommend to use the following parameters to generate the GFF annotation file:
    ```sh
    RepeatMasker -pa ${process_num} -species ${species} -no_is -nolow -norna -poly -html -gff -dir ${output_dir}
    ```
    - Additionally, suggest to remove the lines with low-complexity sequence if the GFF file contains.
    ```sh
    grep -vE "rich|\)n" ${gff_file} > ${filtered_gff_file}
    ```
    - The TE annotation file `<TE.gff>` must be formatted as follows：
    ```
